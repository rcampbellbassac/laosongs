...
export default new Vuex.Store({
  ...
  actions: {
    async initdata({ commit }) {
      localforage.getItem('LaoSongs').then((localstoragedata) => {
        if (localstoragedata != null) {
          ...
          fetch('https://laoadventist-media.s3-us-west-2.amazonaws.com/LaoSongs.json')
            .then((response) => (response.json()))
            .then(async (responseJSON) => {
              await localforage.setItem('LaoSongs', responseJSON)
